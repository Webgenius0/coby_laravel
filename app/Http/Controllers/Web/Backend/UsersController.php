    ...
    public function store(Request $request)
    {
        ...
        try {
            if ($request->hasFile('image')) {
                $validate['image'] = Helper::fileUpload($request->file('image'), 'user', time() . '_' . getFileName($request->file('image')));
            }
            $validate['slug'] = Helper::makeSlug(User::class, $validate['name']);
        ...
        try {
            ...
            if ($request->hasFile('image')) {
                if ($user->image && file_exists(public_path($user->image))) {
                    Helper::fileDelete(public_path($user->image));
                }
                $validate['image'] = Helper::fileUpload($request->file('image'), 'user', time() . '_' . getFileName($request->file('image')));
            }

            $user->update($validate);
            session()->put('t-success', 'User updated successfully');
        } catch (Exception $e) {
            session()->put('t-error', $e->getMessage());
        }
